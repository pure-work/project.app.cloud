...
public class StringUtil extends org.apache.commons.lang.StringUtils {
	...
	public static final String RANDOM_NUM_STR ="0123456789";
	
	public static final String RANDOM_LOW_STR ="abcdefghijklmnopqrstuvwxyz";
	
	public static final String RANDOM_UPPER_STR ="ABCDEFGHIJKLMNOPQRSTUVWXYZ";

	public static boolean isLegalWithNumWord_(String source){
		for(int i = 0;i<source.length();i++){
			String c = source.charAt(i)+"";
			if(!RANDOM_NUM_STR.contains(c)&&!RANDOM_LOW_STR.contains(c)
					&&!RANDOM_UPPER_STR.contains(c)&&!UNDERLINE.equals(c)){
				return false;
			}
		}
		return true;
	...
	public static String toSortPairString(Map<String, Object> map){
		if(map == null) {
			return ""; 
		}
        //使用TreeMap对key进行排序
	...
	public static String toPairString(Map<String, Object> map){
     ...
     */
    public static String toPairString(Map<String, Object> map, boolean ignoreEmptyValue){
        if(map == null) return "";
   ...
   /* 
     * 签名，把map转key-value字符串（value为空的排除），然后md5，然后转大写
    ...
    *//**
     ...
     * @author liubiaocai
     *//*
    public static String sign(Map<String, Object> map, String includeKeys){
        if(map == null) throw new IllegalArgumentException("map");
        //使用TreeMap对key进行排序
        Map<String, Object> treeMap = new TreeMap<String, Object>();
        for (Map.Entry<String, Object> entry: map.entrySet()){
            if(!isNullOrEmpty(entry.getValue()) && (isEmpty(includeKeys) || includeKeys.contains(entry.getKey()))){
            	treeMap.put(entry.getKey(), entry.getValue());
            }
        }
        String keyValuePair = toPairString(treeMap);
        return MD5Utils.getMD5Str(keyValuePair).toUpperCase();
    }
    */
    /*
