...
import com.gozi.core.base.enums.BasicType;
...
public class ObjUtil {
	...
	public static Object parse(Class<?> clazz, Object value) {
		try {
			...
			if (Date.class.isAssignableFrom(clazz)) {
				...
				return DateUtil.getInstance().parse(valueStr);
			...
			// 未找到可转换的类型，返回原值
			return (StringUtil.isBlank(valueStr)) ? null : value;
		}
	}
	...
	public static Object parseBasic(Class<?> clazz, String valueStr) {
		...
		if (StringUtil.isBlank(valueStr)) return null;
	...
	public static Integer[] toIntArray(boolean isIgnoreConvertError, Object... values) {
		...
		for (int i = 0; i < values.length; i++) {
			...
			if (null == v && isIgnoreConvertError == false) {
				throw new BusinessException(StringUtil.format("Convert [%s] to Integer error!", values[i]));
			}
			ints[i] = v;
		}
		return ints;
	...
	public static Long[] toLongArray(boolean isIgnoreConvertError, Object... values) {
		...
		for (int i = 0; i < values.length; i++) {
			...
			if (null == v && isIgnoreConvertError == false) {
				throw new BusinessException(StringUtil.format("Convert [%s] to Long error!", values[i]));
			}
			longs[i] = v;
		}
		return longs;
	...
	public static Double[] toDoubleArray(boolean isIgnoreConvertError, Object... values) {
		...
		for (int i = 0; i < values.length; i++) {
			...
			if (null == v && isIgnoreConvertError == false) {
				throw new  BusinessException(StringUtil.format("Convert [%s] to Double error!", values[i]));
			}
			doubles[i] = v;
		}
		return doubles;
	...
	public static <T> Float[] toFloatArray(boolean isIgnoreConvertError, Object... values) {
		...
		for (int i = 0; i < values.length; i++) {
			...
			if (null == v && isIgnoreConvertError == false) {
				throw new BusinessException(StringUtil.format("Convert [%s] to Float error!", values[i]));
			}
			floats[i] = v;
		}
		return floats;
	...
	public static Boolean toBool(Object value, Boolean defaultValue) {
		...
		if("true".equals(valueStr) || "yes".equals(valueStr) || "1".equals(valueStr))
			return true;
		else
			return false;
	}
	...
	public static Boolean[] toBooleanArray(boolean isIgnoreConvertError, Object... values) {
		...
		for (int i = 0; i < values.length; i++) {
			...
			if (null == v && isIgnoreConvertError == false) {
				throw new BusinessException(StringUtil.format("Convert [%s] to Boolean error!", values[i]));
			}
			bools[i] = v;
		}
		return bools;
	...
	public static Integer getDecimals(Double d) {
		if(d == null) {
			return 0;
		}
		String str = d.toString();
