...
public class BeanUtil {
	...
	public static PropertyDescriptor getPropertyDescriptor(Class<?> clazz, final String fieldName) throws IntrospectionException {
		...
		for (PropertyDescriptor propertyDescriptor : propertyDescriptors) {
			if (ObjUtil.equals(fieldName, propertyDescriptor.getName())) {
				return propertyDescriptor;
			}
		}
		return null;
	...
	public static <T> T fillBean(T bean, ValueProvider valueProvider) {
		...
		try {
			...
			for (PropertyDescriptor property : propertyDescriptors) {
				...
				try {
					property.getWriteMethod().invoke(bean, ObjUtil.parse(property.getPropertyType(), value));
				} catch (Exception e) {
					e.printStackTrace();
				}
			}
	...
	public static <T> Map<String, Object> beanToMap(T bean, boolean isToUnderlineCase,String defaultValue) {
				...
				if (false == key.equals("class")) {
					...
					if (null != value) {
						if(value instanceof Date){
							value =  DateUtil.format((Date)value, DateUtil.DATATIME);
					    }
						map.put(isToUnderlineCase ? StringUtil.toUnderlineCase(key) : key, value);
					}else {
						map.put(isToUnderlineCase ? StringUtil.toUnderlineCase(key) : key, defaultValue);
					}
				}
			}
		} catch (Exception e) {
